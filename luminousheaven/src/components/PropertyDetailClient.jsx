// luminousheaven/src/components/PropertyDetailClient.jsx
"use client";
...
export default function PropertyDetailClient({ initialProperty, identifier }) {
  ...
  const routeIdentifier = identifier || params?.identifier;
  const { refreshUser } = useAuth();

  let resolvedId = initialProperty?.id || null;
  const isStrictlyNumeric = /^\d+$/.test(routeIdentifier);

  if (!resolvedId) {
    const parts = routeIdentifier ? routeIdentifier.split("-") : [];
    const lastPart = parts.length > 0 ? parts[parts.length - 1] : "";

    if (isStrictlyNumeric) {
      resolvedId = routeIdentifier;
    } else if (lastPart && isValidHashId(lastPart)) {
      resolvedId = decodeId(lastPart);
    } else {
      const legacyMatch = routeIdentifier?.match(/^(\d+)/);
      if (legacyMatch) {
  ...
  useEffect(() => {
            ...
            onFreshData: (fetchedProperty) => {
              if (fetchedProperty) {
                ...
                if (fetchedProperty.id && fetchedProperty.title) {
                  ...
                  if (routeIdentifier !== expectedIdentifier && !isStrictlyNumeric) {
    ...
    fetchProperty();
  }, [resolvedId, routeIdentifier, isStrictlyNumeric, router]);
    ...
    <div className="bg-white min-h-screen">
      <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16">
          ...
          <div className="md:hidden -mx-4 sm:-mx-6 relative h-[50vh] sm:h-[60vh] overflow-hidden group mb-8">
            <div className="flex overflow-x-auto snap-x snap-mandatory h-full hide-scrollbar scroll-smooth">
              {galleryImages.map((img, idx) => (
                <div
                  key={idx}
                  className="min-w-full h-full snap-center relative shrink-0 cursor-pointer"
            ...
            {galleryImages.length > 1 && (
              <div className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-md px-3.5 py-1 rounded-full shadow-lg z-10 pointer-events-none">
                <span className="font-sans text-[10px] font-bold tracking-[0.2em] text-brand-dark uppercase">
                  Swipe Gallery ({galleryImages.length})
                </span>
              </div>
            )}
          </div>
            ...
            <div className="sticky top-10 space-y-8">
              ...
              <div className="bg-brand-bg p-6 rounded-lg shadow-sm border border-brand-divider">
                <AgentProfile
                  ...
                  propertyIdentifier={routeIdentifier}
                />
              </div>
            </div>
          </div>
