// luminousheaven/src/components/PropertyCard.jsx
"use client";
...
const PropertyCard = ({ property, onUnsave }) => {
  ...
  const slug = generateSmartSlug(property.id, property.title);
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden bg-brand-bg w-full h-[350px] sm:h-[450px] md:h-[500px]">
        <Link href={`/properties/${slug}`} className="block w-full h-full relative">
          <Image
            src={property.image}
            alt={property.title}
            fill
        ...
        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm text-brand-dark px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] z-10">
      ...
      <div className="pt-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-2">
          <Link href={`/properties/${slug}`} className="block w-2/3">
        ...
        <div className="mt-auto pt-4 border-t border-brand-divider/30 flex justify-between items-center font-sans text-[11px] text-brand-dark uppercase tracking-[0.15em] font-semibold">
          ...
          {property.status && (
            <span
              className={
                property.status === "Sold" ? "text-red-800" : "text-brand-accent"
              }
            >
              {property.status}
            </span>
